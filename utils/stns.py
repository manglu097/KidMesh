...
def stn_quaternion_rotations(params):   # 四元数是一种有效的旋转表示方法

    params = params.view(3)
    qi, qj, qk = params
    # 这里假设params已经包含了四元数的虚部qi, qj, qk，且忽略了实部（因为实部可以从虚部计算得到）。实际上，一个完整的四元数由一个实部和三个虚部组成，这里的处理似乎假设了实部以某种方式被省略或另外处理。
    s = qi ** 2 + qj ** 2 + qk ** 2
