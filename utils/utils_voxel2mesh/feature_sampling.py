...
class LearntNeighbourhoodSampling(nn.Module):
    ...
    def __init__(self, config, features_count, step):
        ...
        D, H, W = config.pad_shape
        self.shape = torch.tensor([W, H, D]).cuda().float()

        self.shift = torch.tensor(list(product((-1, 0, 1), repeat=3)))[None].float() * torch.tensor([[[2 ** (config.steps+1 - step)/(W), 2 ** (config.steps+1 - step)/(H),
                                                                                                       2 ** (config.steps+1 - step)/(D)]]])[None]
        self.shift = self.shift.cuda()
    ...
    def forward(self, voxel_features, vertices):
        ...
        features = F.grid_sample(voxel_features, center, mode='bilinear', padding_mode='border', align_corners=True)  # 1,256,162,1,1
        ...
        features = F.grid_sample(voxel_features, neighbourhood, mode='bilinear', padding_mode='border', align_corners=True)
        features = features[:, :, :, :, 0]
        features = torch.cat([features, neighbourhood.permute(0,4,1,2,3)[:,:,:,:,0]], dim=1)   # 1, 256+3,162,27

        features_diff_from_center = features - features[:,:,:,0][:,:,:,None] # 0 is the index of the center cordinate in shifts
        features_diff_from_center = features_diff_from_center.permute([0,3,2,1])    # 线性层通道数在最后一维
        features_diff_from_center = self.feature_diff_1(features_diff_from_center)
        features_diff_from_center = self.feature_diff_2(features_diff_from_center)
        features_diff_from_center = features_diff_from_center.permute([0,3,2,1])
        
        features_diff_from_center = self.sum_neighbourhood(features_diff_from_center)[:, :, :, 0].transpose(2, 1)    #舍弃邻域点27

        center_feautres =  features[:,:,:,13].transpose(2, 1)     #  为什么是13不是0，可以尝试改
        center_feautres = self.feature_center_1(center_feautres)
        center_feautres = self.feature_center_2(center_feautres)

        features = center_feautres + features_diff_from_center    # 1，162，256
        return features
