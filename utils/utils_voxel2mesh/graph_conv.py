...
class GraphConv(nn.Module):
    ...
    def forward(self, input, A, Dinv, vertices, faces):
        ...
        coeff = torch.bmm(Dinv, A) # row normalization   torch.bmm(): 这个函数执行批量的矩阵乘法。对于每个批次（batch），Dinv和A中对应的矩阵进行矩阵乘法操作。假设Dinv的形状是(b, n, n)，A的形状是(b, n, m)，那么结果张量的形状将会是(b, n, m)，其中b是批次大小，n是节点数（或Dinv的列数），m是A的列数。
        # 通过将度矩阵的逆（或伪逆）与邻接矩阵相乘，可以实现对图中每个节点的特征进行规范化的加权和，这有助于在保持图结构信息的同时，传播和更新节点的特征表示。
        y = self.fc(input)
...
class Feature2VertexLayer(nn.Module):
    ...
    def __init__(self, in_features, hidden_layer_count, batch_norm=False):
        ...
        for i in range(hidden_layer_count, 1, -1):
            self.gconv += [GraphConv(i * in_features // hidden_layer_count, (i-1) * in_features // hidden_layer_count, batch_norm)]    # 图卷积通道在变化
...
class Features2Features(nn.Module):
    ...
    def __init__(self, in_features, out_features, hidden_layer_count=2, graph_conv=GraphConv):
        ...
        for i in range(hidden_layer_count):
            gconv_hidden += [graph_conv(out_features, out_features)]    # 图卷积通道不在变化
...
class Features2FeaturesWithResidual(nn.Module):
    def __init__(self, in_features, out_features, hidden_layer_count=2, graph_conv=GraphConv):
        super(Features2FeaturesWithResidual, self).__init__()

        self.gconv_first = graph_conv(in_features, out_features)
        gconv_hidden = []
        for i in range(hidden_layer_count):
            gconv_hidden += [graph_conv(out_features, out_features)]
        self.gconv_hidden = nn.ModuleList(gconv_hidden)  # Convert to ModuleList for proper handling of parameters
        self.gconv_last = graph_conv(out_features, out_features)
        self.residual_adjust = None
        if in_features != out_features:
            self.residual_adjust = graph_conv(in_features, out_features)

    def forward(self, features, adjacency_matrix, degree_matrix, vertices, faces):


        if self.residual_adjust is not None:
            residual = self.residual_adjust(features, adjacency_matrix, degree_matrix, vertices, faces)

        features = F.relu(self.gconv_first(features, adjacency_matrix, degree_matrix, vertices, faces))
        for gconv_hidden in self.gconv_hidden:
            features = F.relu(gconv_hidden(features, adjacency_matrix, degree_matrix, vertices, faces))  # Add residual connection
        features = features + residual  # Add residual connection from input features
        features = F.relu(features)  # Apply ReLU activation
        return self.gconv_last(features, adjacency_matrix, degree_matrix, vertices, faces)
